...
def right_side(x, t):
    et = 2.0 * t - 1.0
    ex = torch.exp(1.0 / (et**2 + e))
    return 2.0 * ex * (1.0 + 2.0 * et * (x**2 - 1.0) / (et**2 + e)**2)
        ...
        for training_iter in range(1, epochs + 1):
            try:
                ...
                if training_iter % self.print_interval == 0 and self.verbosity:
                    ...
                    print(f"  Relative L2: {training_l2}")
        ...
        for cur_supervised_option in supervised_options:
            ...
            for order in orders:
                for size in training_size:
                    ...
                    # ==== NEW: build u_true, f, g for 1D problem u_t = u_xx + f on x in (-1,1) ====
                    # We use the first coordinate as x; ignore y (it can be e.g. all zeros)

                    # interior + boundary points used for data
                    x_tilde = torch.vstack([x_i, x_b])   # shape (Ni+Nb, 1)

                    Nt = 25  # must match time_range and the 0..24 loop in Trainer
                    time_range = torch.linspace(0.0, 1.0, Nt, dtype=PRECISION, device=device_string).view(Nt, 1, 1)
                    ...
                    for k in range(Nt):
                        # all interior+boundary points at time t_k
                        t_all = time_range[k].expand(x_tilde.shape[0], 1)
                        u_k = analytical(x_tilde, t_all)
                        f_k = right_side(x_tilde, t_all)
                        u_list.append(u_k)
                        f_list.append(f_k)

                        # boundary points for BC at time t_k (u(-1,t)=u(1,t)=0)
                        t_b = time_range[k].expand(x_b.shape[0], 1)
                        g_k = analytical(x_b, t_b)
                        g_list.append(g_k)

                    u_true = torch.cat(u_list, dim=0)  # (Nt*(Ni+Nb), 1)
                    f      = torch.cat(f_list, dim=0)  # same shape
                    g      = torch.cat(g_list, dim=0)  # (Nt*Nb, 1)
